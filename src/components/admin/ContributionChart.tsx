...
import type { Contribution } from "@/lib/firestoreService";
...
function groupByPeriod(
  ...
  filtered.forEach((c) => {
    ...
    grouped[key] = (grouped[key] || 0) + c.value;
  });
...
function CustomTooltip({
  ...
  return (
    <div className="bg-card border border-border rounded-xl px-3 py-2 shadow-md">
      ...
      <p className="text-sm font-bold text-primary">{brl(payload[0].value)}</p>
    </div>
  );
}
...
export function ContributionChart({ contributions }: ContributionChartProps) {
  ...
  const data = useMemo(
    () => groupByPeriod(contributions, period),
    ...
    <div className="bg-card rounded-2xl border border-border/60 shadow-sm p-5">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-5">
        <div>
          <h3 className="font-semibold text-lg text-primary">
            Contribuições por Período
          </h3>
          <p className="text-xs text-muted-foreground mt-0.5">
            Total no período:{" "}
            <span className="font-bold text-foreground">{brl(total)}</span>
          </p>
        </div>
      ...
      {data.length === 0 ? (
        <div className="flex items-center justify-center h-[250px] text-muted-foreground text-sm">
          Nenhuma contribuição no período selecionado.
        </div>
      ) : (
