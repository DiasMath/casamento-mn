...
export function OndeVaiSer() {
  return (
    <section id="casamento" className="relative px-4 py-16 sm:py-24 overflow-hidden" style={{ scrollMarginTop: "80px" }}>
      <Flower className="absolute top-8 -left-4" size={70} variant="yellow" rotate={-15} opacity={0.5} />
      <Flower className="absolute top-20 right-8" size={55} variant="blue" rotate={25} opacity={0.45} />
      <Branch className="absolute top-0 right-0 hidden sm:block" size={120} rotate={165} opacity={0.35} />
      <Flower className="absolute bottom-12 left-4" size={65} variant="mixed" rotate={-30} opacity={0.5} />
      <Flower className="absolute bottom-24 right-6 hidden sm:block" size={45} variant="yellow" rotate={20} opacity={0.4} />
      <Branch className="absolute bottom-0 left-0 hidden sm:block" size={130} rotate={-5} opacity={0.35} />
      <Garland className="absolute -bottom-4 left-1/2 -translate-x-1/2 hidden sm:block" size={250} rotate={180} opacity={0.3} />
          ...
          <div className="bg-card rounded-3xl p-6 sm:p-8 shadow-[var(--shadow-card)] border border-border/60 space-y-5">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-foreground/70">{WEDDING_VENUE.address}</p>
              </div>
            </div>
            <div className="flex items-start gap-3">
